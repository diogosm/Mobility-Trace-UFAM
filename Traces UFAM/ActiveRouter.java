...
public abstract class ActiveRouter extends MessageRouter {
    ...
    /** Limit of times that a message can be sent */
    private final int nDropLimit = 5;
	/** HashMap for N-Drop */
	public HashMap<String, Integer> countN = new HashMap<String, Integer>();
	/** HashMap for Evict*/
	public HashMap<String, Integer> countProb = new HashMap<String, Integer>();
	...
	protected boolean makeRoomForMessage(int size){
		...
		while (freeBuffer < size) {
			...
			//XXX: políticas de gerência de buffer
			//Message m = getOldestMessage(true); // don't remove msgs being sent
			//Message m = nDrop(true, nDropLimit);
			Message m = evictShortestLifeTime(evictLimit); //falta determinar o limite
	...
	/**
	 *  N-Drop Policy
	 *  Returns the message that has achieved the sent limit.
	 */
	protected Message nDrop(boolean excludeMsgBeingSent, int limit) {
		Collection<Message> messages = this.getMessageCollection();
		Message toDrop = null;
		for (Message m : messages) {
		    if (!this.countN.containsKey(m.getId())) {
		        this.countN.put(m.getId(), 0);
		    }
		    
            //if (excludeMsgBeingSent && isSending(m.getId())) {
                this.countN.put(m.getId(), ((countN.get(m.getId()))+ 1) );
                System.out.println("nCount: " + countN.get(m.getId()));
			//}

			if (this.countN.get(m.getId()) == limit && toDrop == null) {
			    toDrop = m;
			}
		}
		
		if (toDrop != null)
		    System.out.println(">>> toDrop: " + toDrop.getId());
		else
		    System.out.println(">>> toDrop NULL");
		    
		return toDrop;
	}

    /**
     * Evict shortest life time first
	 * 
     */
    protected Message evictShortestLifeTime(double limit) {
    	Collection<Message> messages = this.getMessageCollection();
		Message toDrop = null;
		for (Message m : messages) {
		    if (!this.countProb.containsKey(m.getId())) {
		        this.countProb.put(m.getId(), 0);
		    }
		    
            //if (isSending(m.getId())) {
                this.countProb.put(m.getId(), (countProb.get(m.getId())) + (int)(Math.random()*100));
			//}

            //TODO: Testar método
            System.out.println("count: " + countProb.get(m.getId()));

			if (this.countProb.get(m.getId()) >= limit && toDrop == null) {
			    toDrop = m;
			}
		}
