...
public abstract class ActiveRouter extends MessageRouter {
	...
	protected boolean makeRoomForMessage(int size){
		...
		while (freeBuffer < size) {
			Message m = getDropTailMessage(true); // don't remove msgs being sent
	...
	/**
	 * Returna a primeira mensagem presente no buffer
	 * Poe a coleção de mensagens em uma variável do tipo array e captura o primeiro elemento deste
	 */
	protected Message getDropTailMessage(boolean excludeMsgBeingSent) {
		Collection<Message> messages = this.getMessageCollection();
		Message first = null;
		Object[] queueMessages = messages.toArray();
		for (int i=0; i<queueMessages.length; i++) {
			
			if (excludeMsgBeingSent && isSending(((Message) queueMessages[i]).getId())) {
				continue; // skip the message(s) that router is sending
			}
			else{
				first = (Message)queueMessages[i];
				break;
			}
		}
