...
public abstract class ActiveRouter extends MessageRouter {
	...
	protected int startTransfer(Message m, Connection con) {
		...
		if (retVal == RCV_OK) { // started transfer
			...
			m.incForwardCount(); // incrementa o nro de envios
		}
	...
	protected boolean makeRoomForMessage(int size){
		...
		while (freeBuffer < size) {
			Message m = getEvictMostForwardedFirstMessage(true); // don't remove msgs being sent
	...
	/**
	 * Retorna a mensagem presente no buffer que tiver o maior número de envios registrados
	 * O algoritmo navega pela coleção de mensagens e procura alguma que tenha o número de envios registrados forwardCount
	 * maior ou igual ao número máximo MAX_FORWARDED. Caso não encontre, escolhe a mensagem que tiver o número maior.
	 */
	protected Message getEvictMostForwardedFirstMessage(boolean excludeMsgBeingSent) {
		Collection<Message> messages = this.getMessageCollection();
		Message mostForwarded = null;
		int i;
		for (Message m : messages) {
			...
			else if (mostForwarded == null ) {
				mostForwarded = m;
			}
			else if (m.getForwardCount() >= Message.MAX_FORWARDED) {
				mostForwarded = m;
				break;
			}
			else if (mostForwarded.getForwardCount() < m.getForwardCount()) {
				mostForwarded = m;
			}
			
		}
		
		return mostForwarded;
	}
